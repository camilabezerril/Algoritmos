...
public class GeradorDeRelatorios {
	...
	private TipoOrdenacao tipoOrdenacao = null;
	private CritOrdenacao critOrdenacao = null;
	private Filtro tipoFiltro = null;

	public void setTipoOrdenacao(TipoOrdenacao tipoOrdenacao) {
		this.tipoOrdenacao = tipoOrdenacao;
	}
	public void setCritOrdenacao(CritOrdenacao critOrdenacao) {
		this.critOrdenacao = critOrdenacao;
	}
	public void setTipoFiltro(Filtro tipoFiltro) {
		this.tipoFiltro = tipoFiltro;
	}

	public void ordenar(){
		if(this.tipoOrdenacao != null && this.critOrdenacao != null)
			this.tipoOrdenacao.aplicarOrdenacao(produtos, critOrdenacao);
	}

	public boolean filtrar(Produto produto){
		if(this.tipoFiltro != null) return this.tipoFiltro.aplicarFiltro(produto);
		else return true; //Não aplicar filtro é o mesmo que selecionar todos.
	}
	...
	public void geraRelatorio(String arquivoSaida) throws IOException {
		...
		for(int i = 0; i < produtos.length; i++){
			...
			boolean selecionado = this.filtrar(p);
	...
	public static void main(String [] args) {
		...
		gdr.setCritOrdenacao(new peloEstoque("Decrescente"));
		gdr.setTipoOrdenacao(new quickSort());
		gdr.setTipoFiltro(new porEstoque(5));
